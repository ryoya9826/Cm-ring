...
\begin{defi}[Krull次元]\index{#Krullじげん@Krull次元}\label{defi:Kurll次元}
	...
	定義から$\dim A$は$\sup\mkset{\idht P}{P\in\spec A}$である.
...
\begin{thm}\label{thm:有限生成な整域は次元と超越次元が同じ}
...
%
%\begin{proof}
%	$A$が整域なので,適当な$n$について, $P\in\spec K[X_1,\dots,X_n]$が存在して$A=K[X_1,\dots,X_n]/P$とかけている. $\dim A=r$とおくと,明らかに$r\leq n$である. $n$についての帰納法で示す.
%	\begin{step}
%	\item $n=0$のとき. $A=K$となり明らか.
%	
%	\item $n-1$まで正しいとする.
%	\begin{sakura}
%		\item $r=n$のとき. 
%		
%		$\dim A=\coht P=n$であるから, $P=0$が成り立つ.よって$A=K[X_1,\dots,X_n]$となり明らか.
%		
%		\item $r<n$のとき.
%		
%		$P\neq0$であるので,ある$0\neq f\in P$によって$f(x_1,\dots,x_n)=0$である.ここで$y_i=x_i-x_n^{m_i} (1\leq i\leq n, m_i\geq0)$とおく. $x_i=y_i+x_n^{m_i}$であるから;
%		\[f(y_1+x_n^{m_1},y_2+x_n^{m_2},\dots,y_{n-1}+x_n^{m_{n-1}},x_n)=0\]
%		である.ここで$m_i$を十分大きくとっておくと,左辺を展開したときに;
%		\[cx_n^N+\sum_{j=0}^{N-1}g_j(y_1,\dots,y_{n-1})x_n^j=0\quad(0\neq c\in K, g_j\in K[X_1,\dots,X_{n-1}])\]
%		とできる.これは$x_n$が$K[y_1,\dots,y_{n-1}]$上整であることを示しており, $A$は$K[y_1,\dots,y_{n-1}]$上整である.帰納法の仮定を$K[y_1,\dots,y_{n-1}]$に適用して,証明が完了する.
%	\end{sakura}
%	\end{step}
%\end{proof}
...
$A$が鎖状環のとき,次元公式が成り立つ.よって$A$が鎖状環であることを証明しよう.
...
%本書では代数幾何学の慣例に\textbf{習わず},~コンパクトな位相といえば,開被覆は有限部分被覆を持つ位相空間とする.~(慣例的には更にHausdorff性を要求する.そうでない位相空間は擬コンパクト,準コンパクト(quasi-compact)という.)
...
\begin{thm}[Hilbertの零点定理(Nullstellensatz)]\index{#Hilbertのれいてんていり@Hilbertの零点定理}\label{thm:Nullstellensatz}
