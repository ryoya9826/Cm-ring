...
$S$を$A$の非零因子全体の集合とすると,上の$\varphi$は単射であって, $S^{-1}A$を$A$の\textbf{全商環}(total fractional ring)\index{ぜんしょうかん@全商環}という. $A$が整域のとき,これは体になり\textbf{商体}(quotient field)という.
\end{defi}
...
環$A$で,唯1つの極大イデアル$\ideal{m}$を持つものを\textbf{局所環}(local ring)といい,次の判定条件が知られている.\index{きょくしょかん@局所環}
...
\begin{defi}[GCD整域]\index{#GCD整域@GCD整域}
...
\begin{defi}[半局所環]\index{はんきょくしょかん@半局所環}\label{defi:半局所環}
