...
\begin{proof}[\textbf{\ref{thm:DVRの特徴づけ}の証明}]
	...
	\end{sakura}
	の同値性である．
	\TODO[補題5.1.14を使わずにKrullの交叉定理からなんとかできないかな？証明見直そう？]
	\begin{eqv}[6]
		\item 明らか．
...
\section{Krullの次元定理}
\ref{defi:Kurll次元}で環のKrull次元を定義した．素イデアルの長さをもってして環の大きさを計ったわけであるが，この節ではPoincar\'e\textbf{級数}を用いた別の\quo{計りかた}を，次数付きNoether環に，そして\ref{defi:随伴次数環}で定義した随伴次数環を用いてNoether局所環に対して考えよう．
...
\begin{thm}[Krullの標高定理]\index{#Krullのひょうこうていり@Krullの標高定理}
...
\begin{thm}[Auslander--Bachsbaumの定理]\index{#Auslander--Bachsbaumのていり@Auslander--Bachsbaumの定理}
	正則局所環はUFDである．
\end{thm}
...
%
%\begin{proof}
%%	まず$\leq $を示す．$P\in \supp M/aM$が極小であるとする．このとき$P\in\ass M/aM$だから$\ann M\subset P$となるので，$P\in\supp M$である．一方で$P$が$\supp M$で極小，つまり$P\in\ass M$なら$a$が$M$正則だから$a\not\in P$であるから，$P$は$\supp M$で極小ではない．よって$P'\subsetneq P$となる$P'\in\supp M$が存在する．よって$\supp M/aM$の極大な鎖よりも長い$\supp M$の鎖がとれることになり，$\dim M/aM\leq \dim M-1$である．
%	
...
%	
%	容易に$\ann(M/aM)\subset P$なら$\ann M\subset P$かつ$a\in P$であることがわかる．一方で$\ann M\subset P$かつ$a\in P$ならば$M_P\neq0$であり，また$a/1\in A_P$は$M_P$正則であるから$(M/aM)_P=M_P/aM_P$である．ここで$M_P/aM_P=0$すなわち$M_P\subset aM_P$と仮定すると，任意の$0\neq x\in M_P$についてある$y_1\in M_P$が存在して$x=ay_1$とかけ，また$y_1\neq0$である．$y_1$について同様に$y_1=ay_2$とかける$0\neq y_2\in M_P$が存在するから，$x=a^2y_2$である．繰り返して$0\neq x\in\bigcap_{n\in\N}a^nM_P$となり特にこれは$0$でないが，$a/1\in\rad(A_P)=PA_P$であるからKrullの交叉定理（\ref{cor:Krullの交叉定理の系}）に矛盾している．よって$(M/aM)_P\neq0$となり，$P\in\supp M/aM$すなわち$\ann(M/aM)\subset P$である．よって求める等式が示された．
%\end{proof}
