...
\begin{defi}[約元，倍元]
	$A$を環とする．$a,b\in A$について，ある$r\in A$が存在して$a=rb$となるとき$a$は$b$の\textbf{約元}(divisor)，$b$は$a$の\textbf{倍元}(multiple)であるという．
\end{defi}
...
\begin{proof}
	\begin{mrkw}
		...
		\begin{eqv}
			...
			\item $\varphi:\Z\to K;n\to n\cdot1$は$\Q$を経由する準同型に分解する．すなわち$\psi:\Z\to\Q$（単射）と自然な単射$\iota:\Q\to K$を考えると，2つの準同型$\varphi，\iota\circ\psi:\Z\to K$が定まる．しかし，$\Z$からの準同型は$\varphi$に限るので，$\varphi=\iota\circ\psi$となる．よって$\varphi$は単射ある．
		\end{eqv}
